    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="text-center">
            <h3 class="text-2xl font-bold mb-2">Asas Laravel 12 untuk
                Produk</h3>
            <h5 class="text-sm">
                <a href="https://" class="text-blue-600
hover:underline">www.</a>
            </h5>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray200">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h4 class="text-lg font-semibold text-gray-900">Edit
                        Produk</h4>
                ...
                <form action="{{ route('product.update', $product->id) }}" method="POST"
                    enctype="multipart/form-data" class="space-y5">
                    @csrf
                    @method('PUT')
                    <div>
                        ...
                        <div class="mb-3">
                            <img src="{{ asset('/storage/product/' . $product->gambar) }}"
                                class="w-[200px] rounded-lg border border-gray200" alt="{{ $product->tajuk }}">
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="tajuk" value="{{ old('tajuk', $product->tajuk) }}"
...
mb-2">KETERANGAN</label>
                        <textarea name="keterangan" id="keterangan" rows="6"
                            ...
                            placeholder="Masukkan Keterangan Produk">{{ old('keterangan', $product->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="mt-2 text-sm text-red-600">
                                {{ $message }}
                            </div>
    ...
    old('stok', $product->stok) }}" placeholder="Masukkan Stok Produk" class="w-full border border-gray-200 rounded-lg
px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-200">
                            @error('stok')
                                <div class="mt-2 text-sm text-red-600">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="reset"
                            class="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-amber-500 text-white text-sm font-semibold hover:bg-amber-600 transition">
                            RESET
                        </button>
                    </div>
                </form>
